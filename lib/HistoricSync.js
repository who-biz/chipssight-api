...
var BAD_GEN_ERROR = 'Bad genesis block. Network mismatch between Chipssight and chipsd? Insight is configured for:';

var BAD_GEN_ERROR_DB = 'Bad genesis block. Network mismatch between Chipssight and levelDB? Insight is configured for:';
...
function HistoricSync(opts) {
  ...
  this.getBitcoinCoreVersion(function(bitcoinVersion) {
    if (bitcoinVersion > 100000 && !config.forceRPCsync) {
      ...
      info('CHIPS Core version ', bitcoinVersion);
      info('Using RPC sync ');
    }
  });
      ...
      async.doWhilst(
        ...
        function(err) {
          ...
          if (self.blockChainHeight === blockInfo.height ||
            ...
            oldtip = tip;
            if (!tip)
              throw new Error('Previous blockchain tip was not found on chipsd. Please reset Chipssight DB. Tip was:' + tip)
...
HistoricSync.prototype.showSyncStartMessage = function() {
  ...
  info('Got ' + self.height +
    ' blocks in current DB, out of ' + self.blockChainHeight + ' block at chipsd');

  if (self.blockExtractor) {
    info('CHIPS dataDir configured...importing blocks from .dat files');
...
HistoricSync.prototype.checkDBVersion = function(cb) {
  this.sync.txDb.checkVersion02(function(isOk) {
    if (!isOk) {
      console.log('\n#############################\n\n ## Chipssight API DB is older that v0.2. Please resync using:\n $ util/sync.js -D\n More information at Chipssight API\'s Readme.md');
      process.exit(1);
    }
